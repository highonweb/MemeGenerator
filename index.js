...
const font = ()=>{
    document.querySelector('.fontsize').style.fontSize= document.getElementById('font').value + 'px'
}
...
const select = (id, img) => {
  ...
  ctx.drawImage(img, 0, 0, memes[id].width / memes[id].height * canvas.height, canvas.height)
}
const newtext = () => {
...
const matext = (e) => {
  ...
  ctx.miterLimit = 2
  ctx.strokeText(textie, e.clientX + 40, e.clientY + 40)
  ctx.fillStyle = document.getElementById('fill-color').value
  ctx.fillText(textie, e.clientX + 40, e.clientY + 40)
  ctx.restore()
}
